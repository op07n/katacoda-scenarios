...
echo $DISPLAY ; \
(sudo Xvfb :1 -screen 0 1024x768x24 &) ; (sleep 5) ; (x11vnc -display :1 -nopw -listen localhost -xkb -ncache 10 -ncache_cr -forever &) ; \
(cd novnc && ./utils/launch.sh --vnc localhost:5900 --listen 8080 ); \
git clone https://github.com/nwutils/nw-local-server-example.git; \
npm install; \
npm start;
